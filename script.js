window.onload = () => {
  ...
  let all_ingredients = recipes
    ...
    .flat();
  let unique_ingredients = all_ingredients.filter(
    (ingredient, index) => all_ingredients.indexOf(ingredient) === index
  );
  ...
  for (let i = 0; i < unique_ingredients.length; i++) {
    ...
    document.getElementById(
      "ingredients"
    ).innerHTML += `<li class="list-group-item" id="${ingredient}">${ingredient}</li>`;
  }

  let all_appliance = recipes.map((recipe) => recipe.appliance);
  let unique_appliance = all_appliance.filter(
    (appliance, index) => all_appliance.indexOf(appliance) === index
  );
  ...
  for (let i = 0; i < unique_appliance.length; i++) {
    ...
    document.getElementById(
      "appliances"
    ).innerHTML += `<li class="list-group-item" id="${appliance}">${appliance}</li>`;
  }

  let ustensils = recipes.map((recipe) => recipe.ustensils);
  let all_ustensils = ustensils.flat();
  let unique_ustensils = all_ustensils.filter(
    (ustensil, index) => all_ustensils.indexOf(ustensil) === index
  );
  unique_ustensils = unique_ustensils.map(
  ...
  for (let i = 0; i < unique_ustensils.length; i++) {
    ...
    document.getElementById(
      "ustensils"
    ).innerHTML += `<li class="list-group-item" id="${ustensil}">${ustensil}</li>`;
  }
  ...
  document.getElementById("search").addEventListener("keyup", function () {
    ...
    let filtered_recipes = recipes.filter(
      (recipe) =>
        ...
        recipe.appliance.toLowerCase().includes(search.toLowerCase()) ||
        recipe.ustensils.join(" ").toLowerCase().includes(search.toLowerCase())
    );
    ...
    if (filtered_recipes.length === 0 || search.length < 3) {
      ...
      let filtered_ingredients = filtered_recipes
        .map((recipe) =>
          recipe.ingredients.map((ingredient) => ingredient.ingredient)
        )
        .flat();
      let filtered_unique_ingredients = filtered_ingredients.filter(
        (ingredient, index) =>
          filtered_ingredients.indexOf(ingredient) === index
      );
      ...
      let filtered_appliance = filtered_recipes.map(
        (recipe) => recipe.appliance
      );
      let filtered_unique_appliance = filtered_appliance.filter(
        (appliance, index) => filtered_appliance.indexOf(appliance) === index
      );
      ...
      let filtered_ustensils = filtered_recipes.map(
        (recipe) => recipe.ustensils
      );
      let filtered_all_ustensils = filtered_ustensils.flat();
      let filtered_unique_ustensils = filtered_all_ustensils.filter(
        (ustensil, index) => filtered_all_ustensils.indexOf(ustensil) === index
      );
      filtered_unique_ustensils = filtered_unique_ustensils.map(
        (ustensil) => ustensil.charAt(0).toUpperCase() + ustensil.slice(1)
      );
  ...
  document
    ...
    .addEventListener("keyup", function (event) {
      ...
      let filtered_ingredients = unique_ingredients.filter((ingredient) =>
  ...
  document
    .getElementById("searchAppliances")
    .addEventListener("keyup", function (event) {
      ...
      let filtered_appliances = unique_appliance.filter((appliance) =>
  ...
  document
    .getElementById("searchUstensils")
    .addEventListener("keyup", function (event) {
      ...
      let filtered_ustensils = unique_ustensils.filter((ustensil) =>
  ...
  document
    ...
    .addEventListener("click", function (event) {
      if (event.target.tagName === "SPAN") {
        ...
        let tagClass = event.target.classList[2];
        if (tagClass === "tag-ingredients") {
          let index = unique_ingredients.indexOf(tag);
          unique_ingredients.splice(index, 1);
          document.getElementById("ingredients").innerHTML = "";
          for (let i = 0; i < unique_ingredients.length; i++) {
            let ingredient = unique_ingredients[i];
        ...
        } else if (tagClass === "tag-appliances") {
          let index = unique_appliance.indexOf(tag);
          unique_appliance.splice(index, 1);
          document.getElementById("appliances").innerHTML = "";
          for (let i = 0; i < unique_appliance.length; i++) {
            let appliance = unique_appliance[i];
            document.getElementById(
        ...
        } else if (tagClass === "tag-ustensils") {
          let index = unique_ustensils.indexOf(tag);
          unique_ustensils.splice(index, 1);
          document.getElementById("ustensils").innerHTML = "";
          for (let i = 0; i < unique_ustensils.length; i++) {
            let ustensil = unique_ustensils[i];
            document.getElementById(
